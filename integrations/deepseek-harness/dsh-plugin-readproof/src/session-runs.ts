...
interface SessionState {
  runId: string
  /**
   * 1 for the session's first run. A run id can only be committed once and a
   * committed run refuses further mounts (internal/run/run.go), so a session
   * that keeps reading after its run was committed continues in the next
   * epoch: `dsh-<session>-2`, `-3`, …
   */
  epoch: number
  entries: number
  open: boolean
...
export class SessionRuns {
   ...
   */
  async mount(sessionId: string, uri: string): Promise<{ result: ResolveResult; mount: SessionMount }> {
   ...
   */
  async commit(sessionId: string): Promise<Manifest | undefined> {
    return this.serialize(sessionId, async () => {
      const state = this.states.get(sessionId)
      if (!state || !state.open) return undefined
      state.open = false
      if (state.entries === 0) return undefined
  ...
  async commitIfOwned(target: string): Promise<Manifest | undefined> {
    for (const [sessionId, state] of this.states) {
      if (state.runId === target && state.open) return this.commit(sessionId)
    }
    return undefined
  }
  ...
  private async open(sessionId: string): Promise<SessionState> {
   ...
   */
  private serialize<T>(sessionId: string, body: () => Promise<T>): Promise<T> {

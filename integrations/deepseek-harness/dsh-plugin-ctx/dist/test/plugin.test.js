...
import { merkleRoot } from '@ctx/sdk';
import { TOOL_BASE_NAMES } from '../src/index.js';
import { DEMO_URI, fakeAgent, field, list, renderContent, startApp, startFixture, str, } from './support.js';
...
describe('dsh-plugin-ctx against a real ctxd', () => {
    ...
    it('registers the thirteen Ctx tools on ctx.tools', () => {
        ...
        assert.deepEqual([...TOOL_BASE_NAMES].map((base) => `ctx_${base}`).sort(), [
            'ctx_diff',
            'ctx_evidence_export',
            'ctx_history',
            'ctx_manifest',
...
async function eventually(body, timeoutMs = 5_000) {

...
export async function apply(ctx, config) {
    ...
    async function commit(sessionId) {
        ...
        try {
            ...
            if (manifest)
                log.info(`session ${sessionId} committed as manifest ${manifest.manifest_id}`);
        }
        catch (err) {
    ...
    log.info(`Ctx tools registered against ${endpoint}` +
    ...
    return async () => {
        if (sessions)
            await sessions.commitAll();
        if (spawned)
            await spawned.stop();
    };
}
/** The prose a model reads before it has called anything. */
function promptSection(prefix, sessionRuns) {
    ...
    if (sessionRuns) {
        lines.push('', `Every \`${t('resolve')}\` in this session is already being recorded in a run of its own; the result tells you which. You only need \`${t('run_start')}\` for a run you want to scope yourself.`);
    }
    return lines.join('\n');
}

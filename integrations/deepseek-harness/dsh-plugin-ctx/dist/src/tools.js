import { buildEvidence, CtxError } from '@ctx/sdk';
import { defineTool } from '@deepseek-ai/dsh-tools';
import { encodeContent } from './content.js';
import { commitRun, mountRun, startRun } from './ctx-client.js';
import { diffOut, manifestOut, replayOut, resolveOut, resourceInfo, snapshotInfo, tagInfo, toJson, } from './payloads.js';
...
export function registerTools(ctx, deps) {
    ...
    const t = (base) => `${config.toolPrefix}${base}`;
    ...
    ctx.tools.register(defineTool({
        ...
        parameters: {
            uri: {
                type: 'string',
                required: true,
                description: 'the resource to read, ctx://<namespace>/<path>; append @<tag> (e.g. ctx://acme/policies/refunds@prod) to pin exactly the snapshot that tag names',
            },
        },
        output: {
    ...
    ctx.tools.register(defineTool({
        ...
        output: {
            ...
            render: (_args, value) => blocks(`${count(read(value).snapshots, 'snapshot')} recorded for ${text(read(value).uri)}.`, value),
        },
        async execute(args) {
    ...
    ctx.tools.register(defineTool({
        ...
        output: {
            ...
            render: (args, value) => blocks(`Mounted ${args.uri} into run ${args.run_id} at position ${number(read(value).position)}.`, value),
        },
        async execute(args) {
    ...
    ctx.tools.register(defineTool({
        ...
        output: {
            ...
            render: (_args, value) => {
                const v = read(value);
                return blocks(`${number(v.changed)} changed, ${number(v.added)} added, ${number(v.removed)} removed, ${number(v.unchanged)} unchanged.`, value);
            },
        },
        async execute(args) {
    ...
    ctx.tools.register(defineTool({
        ...
        output: {
            ...
            render: (_args, value) => {
                ...
                return blocks(`${ok ? 'Replay verified' : 'REPLAY MISMATCH'}: ${count(v.entries, 'entry', 'entries')} in manifest ${text(v.manifest_id)}.`, value);
            },
        },
        async execute(args) {
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args) {
            const uri = bareUri(args.uri);
            return attempt(`set tag ${uri}@${args.tag}`, async () => toJson(tagInfo(await client.setTag(uri, args.tag, args.snapshot_id))));
        },
    }));
    ctx.tools.register(defineTool({
        name: t('tag_list'),
        ...
        output: {
            ...
            render: (_args, value) => blocks(`${count(read(value).tags, 'tag')} on ${text(read(value).uri)}.`, value),
        },
        async execute(args) {
            const uri = bareUri(args.uri);
    ...
    ctx.tools.register(defineTool({
        ...
        parameters: {
            ...
            with_content: {
                type: 'boolean',
                description: 'embed each entry’s bytes in the bundle as base64; off by default so the bundle proves what was read without disclosing it',
            },
        },
        output: {
...
function inline(content, contentHash, config) {
...
function blocks(headline, value) {
    return [
        { type: 'text', text: headline },
        { type: 'text', text: JSON.stringify(value, null, 2) },
    ];
}
function resolveHeadline(uri, value) {
    const v = read(value);
...
function manifestHeadline(value) {
    const v = read(value);

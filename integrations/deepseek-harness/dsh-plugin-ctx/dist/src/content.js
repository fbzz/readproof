 ...
 */
function truncationMarker(shown, total, contentHash) {
 ...
 * prefix.
 */
export function encodeContent(content, contentHash, maxBytes) {
...
function trimToRuneBoundary(buf) {
    ...
    while (lead >= 0 && lead > buf.length - 4) {
        const b = buf.at(lead);
        if (b === undefined || (b & 0b1100_0000) !== 0b1000_0000)
            break;
        lead--;
    }
    ...
    if (lead + sequenceLength(b) <= buf.length)
        return buf;

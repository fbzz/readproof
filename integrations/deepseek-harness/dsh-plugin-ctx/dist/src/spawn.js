 ...
 */
export async function spawnCtxd(config, log) {
    ...
    const exited = new Promise((_resolve, reject) => {
        child.once('error', (err) => reject(new Error(`ctx: could not start ${config.ctxdPath}: ${err.message}`)));
        child.once('exit', (code, signal) => reject(new Error(`ctx: ${config.ctxdPath} exited before becoming healthy (code ${code}, signal ${signal})`)));
    });
    try {
...
export function endpointForAddr(addr) {

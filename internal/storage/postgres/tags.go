...
func (s *TagStore) Set(ctx context.Context, t tag.Tag) error {
	...
	var owner string
	err := s.DB.QueryRowContext(ctx, `SELECT resource_uri FROM snapshots WHERE snapshot_id = $1`, t.SnapshotID).Scan(&owner)
	if errors.Is(err, sql.ErrNoRows) {
		return fmt.Errorf("postgres: set tag: %w: %s", snapshot.ErrNotFound, t.SnapshotID)
	}
	if err != nil {
		return fmt.Errorf("postgres: set tag: look up snapshot: %w", err)
	}
	...
	_, err = s.DB.ExecContext(ctx, `
		INSERT INTO tags (resource_uri, tag, snapshot_id, updated_at) VALUES ($1, $2, $3, $4)
	...
	if err != nil {
		return fmt.Errorf("postgres: set tag: %w", err)
	}
	return nil
}
...
func (s *TagStore) Get(ctx context.Context, uri, name string) (tag.Tag, error) {
	row := s.DB.QueryRowContext(ctx, `
		SELECT resource_uri, tag, snapshot_id, updated_at FROM tags WHERE resource_uri = $1 AND tag = $2`, uri, name)
	t, err := scanTag(row)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return tag.Tag{}, fmt.Errorf("%w: %s@%s", ErrTagNotFound, uri, name)
		}
		return tag.Tag{}, fmt.Errorf("postgres: get tag: %w", err)
	}
	return t, nil
}
...
func (s *TagStore) List(ctx context.Context, uri string) ([]tag.Tag, error) {
	rows, err := s.DB.QueryContext(ctx, `
		SELECT resource_uri, tag, snapshot_id, updated_at FROM tags WHERE resource_uri = $1 ORDER BY tag ASC`, uri)
	if err != nil {
...
func (s *TagStore) Delete(ctx context.Context, uri, name string) error {
	res, err := s.DB.ExecContext(ctx, `DELETE FROM tags WHERE resource_uri = $1 AND tag = $2`, uri, name)
	if err != nil {
